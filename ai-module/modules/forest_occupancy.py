...
class TreeOcupancy:
    ...
    def save_png_model(self, clf, features, estimators_index = 0):
        fig = plt.figure(figsize=(25,20))
        _ = tree.plot_tree(clf.estimators_[estimators_index], 
    ...
    def train_model(self):
        ...
        clf = RandomForestClassifier(max_depth=4, criterion='entropy', max_features=0.6, n_estimators=100, random_state=100)
        clf.fit(X_train, y_train)
        predictions = clf.predict(X_test)

        print(accuracy_score(y_test, predictions))
        self.save_png_model(clf, self.get_features(X_train_), 1)
